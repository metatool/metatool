...
namespace Metaseed.Script
{
    static class Program
    {
        static void Main(string[] args)
        {
            ...
            p.RestoreCompleted += restoreResult =>
            {
                ...
                var o = ScriptOptions.Default.WithReferences(restoreResult.RuntimeReferences.Select(ap => MetadataReference.CreateFromFile(ap)).Concat(new []{typeof(Console).Assembly, typeof(int).Assembly, typeof(Regex).Assembly, typeof(System.Linq.Enumerable).Assembly}.Select(m=>MetadataReference.CreateFromFile(m.Location)))).WithImports(new[]{
                    "System",
                    "System.Threading",
